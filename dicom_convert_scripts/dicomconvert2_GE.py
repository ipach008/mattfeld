...
def convert_dicoms(subjs, dicom_dir_template, outdir, heuristic_file, converter, queue=None):
    for sid in subjs:
        if queue:
            ...
            outcmd = 'bsub -J psb6351-%s -q %s -e /scratch/PSB6351_2017/crash/mattfeld/dcm_err2_%s -o /scratch/PSB6351_2017/crash/mattfeld/dcm_out2_%s < %s' % (sid, queue, sid, sid, script_file)
            os.system(outcmd)
            continue
